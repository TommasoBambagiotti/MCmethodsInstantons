...
def cooled_monte_carlo(
    ...
    for i_minimum in range(2, n_minima):
        ...
        # Equilibration cycle
        with open(output_path + '/n_instantons_final.txt', 'w') as n_writer:
            ...
            for i_mc in range(n_mc_sweeps - n_equil):
                ...
                if (i_mc % n_sweeps_btw_cooling) == 0:
                    ...
                    for i_cooling in range(n_cooling_sweeps):
                        cool.configuration_cooling(x_cold_config,
                                                  ip.x_potential_minimum)
    
                        # Find instantons and antiinstantons
                        n_instantons, n_anti_instantons = cool.find_instantons(x_cold_config,
                                                                                  n_lattice,
                                                                                  ip.dtau)
                        if(i_cooling == n_cooling_sweeps -1):
                            n_writer.write(str(n_instantons) + '\n')
