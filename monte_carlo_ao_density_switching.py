...
def instantons_density_switching(n_lattice,
    ...
    This function computes full quantum corrections to the Gaussian (1-loop) approximation of the intantons density
    using adiabatic switching. The adiabatic switching is done starting from the Gaussian approximation of the action.
    ...
    ----------
    The total length of the euclidean time n_lattice*dtau should not be chosen too large to avoid transition between
    the one instanton sector and the three, five, etc. sector. Similarly the potential minima should not be too small.
    Reasonable values are first_minimum=1.4 and n_lattice*dtau=100.
    """
    ...
    for i_minimum in range(n_minima):
        ...
        gauss_density = 8 * np.power(x_potential_minimum, 5 / 2) \
                        * np.sqrt(2 / np.pi) * np.exp(-4 * np.power(x_potential_minimum, 3) / 3)
        ...
        x_config_one_sector = mc.initialize_lattice(n_lattice, x_potential_minimum, False, True)

        # we expand the action about this configuration (central point in a taylor polyn.)
        x_config_0_one = mc.initialize_lattice(n_lattice, x_potential_minimum, False, True)

        second_der_0_one = mc.second_derivative_action(x_config_0_one[1:-1], x_potential_minimum)
        ...
        second_der_0_zero = mc.second_derivative_action(x_config_0_zero[1:-1], x_potential_minimum)
        ...
        for i_sector in [0, 1]:
            ...
            for i_switching in range(2 * n_switching + 1):
                ...
                for i_mc in range(n_equil, n_mc_sweeps):
                    ...
                    delta_s_alpha_temp += dtau * np.sum(potential_1 - potential_0)

                    delta_s_alpha[i_sector, i_switching] += delta_s_alpha_temp / beta
                    # error
                    delta_s2_alpha[i_sector, i_switching] += np.square(delta_s_alpha_temp) / beta

                    if i_mc == n_mc_sweeps - 1:
                        print(f'delta V alpha {i_sector} for {i_switching}:')
                        print(delta_s_alpha[i_sector, i_switching] / (n_mc_sweeps - n_equil))
    ...
    with open(output_path + '/total_density.txt', 'w', encoding='utf8') as f_writer:
        np.savetxt(f_writer, total_density)

    with open(output_path + '/total_density_err.txt', 'w', encoding='utf8') as f_writer:
        np.savetxt(f_writer, total_density_error)

    with open(output_path + '/potential_minima.txt', 'w', encoding='utf8') as pot_writer:
