...
def monte_carlo_ao(x_potential_minimum,  # potential well position
    ...
    x_config = mc.initialize_lattice(n_lattice, i_cold, x_potential_minimum)

    # Monte Carlo sweeps: Principal cycle

    # Equilibration cycle
    for i_equil in range(n_equil):
        x_config = mc.metropolis_question(x_config, dtau, delta_x, x_potential_minimum)
    ...
    derivative_log_corr_funct, derivative_log_corr_funct_err = \
        mc.log_central_der_alg(x_cor_av, x_cor_err, dtau)
    ...
    derivative_log_corr_funct_3, derivative_log_corr_funct_3_err = \
        mc.log_central_der_alg(x_cor_3_av, x_cor_3_err, dtau)
