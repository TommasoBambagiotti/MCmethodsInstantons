...
def anharmonic_oscillator_diag(f, n_dim, w0):
    ...
    # WE consider also the positive eigenvalues: for some reason the first 4 e are < 0. So we neglect the first 4 eigenvectors and consider the groundstate v[:,4]=> because the eigenvectors are the columns
    ...
    ### We use the convenction: a+ = (mw/2h)^1/2 (x^ +i/mw p^) and a = (mw/2h)^1/2 (x^ - i/mw p^)
    for n in range(n_dim):
        # C_n = | <0|x|n> |^2
        # D_n = | <0|x^2|n> |^2
        # E_n = | <0|x^3|n> |^2
